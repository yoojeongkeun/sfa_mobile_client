...
(function() {
	...
	$.extend(bizMOBRender.prototype, 
	{
		...
		render:function(root, data, dir, options)
		{
			...
			if(that.options.clone)
			{
				var newId;
				var newBox;
				if(that.options.isClass){
					newBox = that.options.newId;
				}else{
					newId = that.options.newId;
					newBox = $("#" + newId);
				}				
	...
	////////////////////////////////////////////////
	function bizMOBMerge()
	{ 
		this.options = {};
		this.template = undefined;
	};
	$.extend(bizMOBMerge.prototype, 
	{
		defaultOptions : 
		{
			clone : false, // 대상을 복사한 후 render 
		...
		addLoopElement:function(parent, target, value, detail, key, newId, targetClassName)
		{
			var that = this;
			var template = $(target, parent).size()>1 ? $(target, that.template) : $(target, parent);
			if(template.size()==0) bizMOB._warning("bizMOBMerge - invalid taret selector. target selector : " + target);
			$.each(value, function(index, element)
			{				
				var target = template.clone();				
				that.parseDir(target, this, detail, key, newId, targetClassName, {index : index}).appendTo(template.parent());
			});
			template.remove();
			return parent;
		},
		addSingleElement:function(parent, target, valueType, value)
		{
			...
			if (targetSplit.length < 3) 
			{
				...
				{
					bizMOB._warning("bizMOBMerge - invalid taret selector. target selector : " + target);
					return;
				}
				if(element.size()==0) bizMOB._warning("bizMOBMerge - target is empty. target : " + targetSplit[0]);  
				if (targetSplit.length == 2) 
				{
					if(!targetSplit[1]) 
					{
						bizMOB._warning("bizMOBMerge - attr is null. attr : " + targetSplit[1] + "\ntarget :" + target);
						return;
					}
					switch (method)
					{
			...
			else {bizMOB._warning("bizMOBMerge - syntax error. target : " + target + "(value : " + value + ")"); }
			return parent;
		},
		parseDir: function(parent, data, paths, key, newId, targetClassName, options)
		{
			...
			$.each(paths, function(index,element)
			{
				if(!this.target) {bizMOB._warning("bizMOBMerge - reference error. target : " + this.target + "(value : " + this.value + ")");}
				else if(!this.value) {bizMOB._warning("bizMOBMerge - reference error. value : " + this.value + "(target : " + this.target + ")");}
				var valueType = this.valueType ? this.valueType : "text";
				var realValue;
				if (this.value.constructor == Function) 
				{
				...
				{
					realValue = eval("data." + this.value);
					if(realValue==undefined) bizMOB._warning("bizMOBMerge - invalid value. value : " + this.value);
				}
								
				switch(this.type)
				{
					case that.TYPE_SINGLE :
						var attrSelector = that.makeKeyData(data, key);
						if($("#" + newId + " " + targetClassName + attrSelector).length > 0){
							idDuplicate = true;
							return;
						}
						
						if(this.value==undefined) {bizMOB._warning("bizMOBMerge - reference error. value : " + this.value + "(target : " + this.target + ")");}
						that.addSingleElement(parent, this.target, valueType, realValue);
						break;
					case that.TYPE_LOOP :
						if(this.detail==undefined) {bizMOB._warning("bizMOBMerge - reference error. detail : " + this.detail + "(value :" + this.value + ")");}
						that.addLoopElement(parent, this.target, realValue, this.detail, key, newId, targetClassName);
						break;
					default :
						bizMOB._warning("bizMOBMerge - unknown type. type : " + this.type);
				}
			});
			if(!idDuplicate){
				return parent;
		...
		merge:function(root, data, dir, key, options)
		{
			if(data==undefined ) { bizMOB._throwError("bizMOBMerge - reference error. data : " + data);}
			if(data.constructor != Object && data.constructor != Array) {bizMOB._throwError("bizMOBMerge - type error. data type : " + data.constructor);}
			if(dir==undefined) { bizMOB._throwError("bizMOBMerge - reference error. dir : " + dir); }
			if(dir.constructor != Array) {bizMOB._throwError("bizMOBMerge - type error. dir type : " + dir.constructor);}
			...
			var targetClassName = dir[0].target;
			if(that.options.clone) target = root.clone();
			else target = root;	
			that.parseDir(target, data, dir, key, newId, targetClassName);
		...
		makeKeyData: function(data, key){
			var returnVal = "";
			for(var i = 0; i < key.length; i++){
				returnVal += "[" + key[i] + "='" + eval("data." + key[i]) + "']";
			}
			return returnVal;
		}
	});
	bizMOB.Util.Merge = new bizMOBMerge();
	...
	*/
	...
	bizMOBResource.prototype.getResource = function(filename){};
...
//			});
	...
	// 장우제 사원 추가 (2014-08-04)
	$.fn.bMMerge = function()
	{
		var inst = bizMOB.Util.Merge._getInstance(this);
		inst.execute.apply(inst, [this].concat(Array.prototype.slice.call(arguments, 0)));
	};
